...
5. Keep HTTP request objects in `web` only. Let `business` orchestrate use cases. Push strong business rules into `domain`. Keep `persistence` focused on data access and raw write results only; do not let Repo or RepoImpl throw business exceptions for update-failed, not-found, or status-illegal semantics.
...
14. For guard-style checks such as null existence, status legality, duplicate submission, active-task conflicts, and single-row update success, prefer unified validation via Apache Commons Lang3 `Validate.notNull(...)`, `Validate.isTrue(...)`, or equivalent unified helpers. For ordinary conditional composition, prefer utility methods over hand-written null and blank chains: use `Objects.nonNull(...)` / `Objects.isNull(...)` for null checks, `StringUtils.isNotBlank(...)` / `StringUtils.isBlank(...)` or `StringUtils.isNotEmpty(...)` / `StringUtils.isEmpty(...)` for strings, and a single project-level collection helper such as `CollectionUtils.isEmpty(...)` or `CollUtil.isEmpty(...)` for collections. Do not scatter raw `if (...) { throw ... }` for simple precondition failures, and do not repeatedly handwrite patterns such as `x != null && !x.isBlank()`. Repo write methods should usually return `boolean`, affected-row count, or saved entity identity; ServiceImpl is responsible for turning those results into business failure semantics. Validation annotation messages should use business-facing Chinese such as `项目不能为空` or `时间轴不能为空`, not field-name-oriented wording such as `projectId不能为空` or `timelineDslJson不能为空`.
