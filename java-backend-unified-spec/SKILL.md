...
14. For guard-style checks such as null existence, status legality, duplicate submission, and active-task conflicts, prefer unified validation via Apache Commons Lang3 `Validate.notNull(...)`, `Validate.isTrue(...)`, or equivalent unified helpers. Do not scatter raw `if (...) { throw ... }` for simple precondition failures. Validation annotation messages should use business-facing Chinese such as `项目不能为空` or `时间轴不能为空`, not field-name-oriented wording such as `projectId不能为空` or `timelineDslJson不能为空`.
