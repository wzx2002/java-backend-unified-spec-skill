...
14. For simple guard-style checks such as basic null existence, duplicate submission, active-task conflicts, and straightforward preconditions, prefer unified validation via Apache Commons Lang3 `Validate.notNull(...)`, `Validate.isTrue(...)`, or equivalent unified helpers. For ordinary conditional composition, prefer utility methods over hand-written null and blank chains: use `java.util.Objects.nonNull(...)` / `java.util.Objects.isNull(...)` for null checks, `org.apache.commons.lang3.StringUtils.isNotBlank(...)` / `isBlank(...)` or `isNotEmpty(...)` / `isEmpty(...)` for strings, and a single project-level collection helper chosen explicitly in the module, such as `org.springframework.util.CollectionUtils.isEmpty(...)` or `cn.hutool.core.collection.CollUtil.isEmpty(...)`, for collections. Do not scatter raw `if (...) { throw ... }` for simple precondition failures, and do not repeatedly handwrite patterns such as `x != null && !x.isBlank()`. For failures that must expose stable business error codes, such as not-found, update-failed, or status-illegal semantics, use `BizException` or a project-level business assert/helper instead of naked `Validate`. Repo write methods should usually return `boolean`, affected-row count, saved entity identity, or typed result; `ServiceImpl` is responsible for turning those results into business failure semantics. Validation annotation messages should use business-facing Chinese such as `项目不能为空` or `时间轴不能为空`, not field-name-oriented wording such as `projectId不能为空` or `timelineDslJson不能为空`.
