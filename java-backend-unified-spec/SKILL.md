...
1. Split the work by project goal or business requirement into modules first. Even if the project finally lands in one Maven module and one `src/main/java`, still split packages by business module first and layer inside each module. Do not flatten a single `src` into one global `controller / service / repo / mapper / dto` structure.
...
12. When Codex generates or modifies Java backend code, Chinese comments are mandatory for classes, `public` / `protected` methods, Controller classes, public endpoint methods, Repo methods, Convert methods, key business fields, and non-obvious business branches. Controller annotations such as `@PostMapping` or `@GetMapping` do not replace method comments. Field comments should explain clear business meaning, units, status semantics, source, or usage boundary when the name alone is not enough.
...
15. Delete useless directories, empty placeholder directories, abandoned demo directories, and temporary package trees directly. Do not keep `temp`, `tmp`, `demo`, `test2`, `misc`, `backup`, or catch-all `common`-style directories unless they have a stable and reviewable responsibility.
16. Maintain repository hygiene and `.gitignore` intentionally. Do not commit IDE files, editor caches, build outputs, temporary debug files, one-off export artifacts, local environment files, local secret material, downloaded credentials, or ad hoc scripts that are not part of the project deliverable. If a file is only for local troubleshooting and should not participate in long-term collaboration, delete it or ignore it instead of leaving it in the repository.
17. After each module is completed, regression test all APIs in that module, not only the endpoints changed in the current task.
