...
12. For pure carrier objects such as `Request` / `Response` / `Command` / `Query` / `VO`, prefer annotations such as `@Getter`, `@Setter`, `@Builder`, `@NoArgsConstructor`, and `@AllArgsConstructor` to remove boilerplate accessors. Do not handwrite trivial getter/setter methods unless framework constraints or non-trivial logic make them necessary.
